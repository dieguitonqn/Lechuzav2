...
from src.modules.documents.domain.interfaces import IDocumentsRepository, IDocsFilesRepository, ITransmittalNPRepository
from src.modules.documents.application.dtos import DocumentToSaveFile, ingresoDocsDTO, RawUploadDataDTO, DocumentDTO, TtalNPDTO
from src.domain.entities.ttals_nps import Transmittal_NP


class DocumentsUC:
    def __init__(self, document_repo: IDocumentsRepository, files_repo:IDocsFilesRepository, ttal_repo: ITransmittalNPRepository):
    ...
    async def _validate_and_process_raw_data(self, raw_data: RawUploadDataDTO, user: User) -> ingresoDocsDTO:
        ...
        self._validate_required_fields(
            obra_id, obra_descripcion, obra_slug, np_ttal, np_ttal_descripcion, np_ttal_file
        )
    ...
    def _validate_required_fields(self, obra_id, obra_descripcion, obra_slug, np_ttal, np_ttal_descripcion, np_ttal_file):
    ...
    def _validate_document_fields(self, codigo, revision, descripcion, file, doc_number):
    ...
    async def _process_validated_documents(self, ingreso_dto: ingresoDocsDTO, user: User):
        ...
        np_ttal_file_path: str = await self.ttal_repo.save_transmittal_np_file(ttal_dto=
            ttal_np_dto, user=user, dirpath=dir_path
        )
        ...
        for doc in ingreso_dto.documentos:
            ...
            file_path = await self.files_repo.save_file(document_to_save, user, dirpath=dir_path)
            ...
            document_saved : Document = self.document_repo.save_document_toDB(document_toDB, user)
