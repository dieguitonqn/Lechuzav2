...
def test_create_ttal_np_doc(client, db_session):
    ...
    if not default_status:
        default_status = Status(id=1, nombre="EN REVISION", descripcion="Documento en proceso de revisión")
    ...
    response = client.post(
        ...
        files=[
            ...
            # Archivos
            ("ttal_np_file", ("ttal_test.pdf", b"Test ttal file content", "application/pdf")),
 ...
 #Creo un proyecto de prueba
    ...
    response = client.post(
        ...
        files=[
            ...
            ("ttal_np_description", (None, "Test Transmittal NP Document Missing Fields")),
            ("ttal_np_file", ("ttal_test.pdf", b"Test ttal file content", "application/pdf")),
            ("document_file", ("doc1.pdf", b"Test doc 1 content", "application/pdf")),
        ],
    )
...
def test_create_ttal_exception(client, db_session):
 ...
 #Creo un proyecto de prueba
    ...
    response = client.post(
        "/api/v1/ttal-docs/",
        files=[
            ("project_id", (None, str(project.id))),  # UUID inválido para forzar error
            ("ttal_np_code", (None, "TTALNP001")),  # Código duplicado
            ("ttal_np_description", (None, "Test Transmittal NP Document Exception")),
            ("ttal_np_file", ("ttal_test.pdf", b"Test ttal file content", "application/pdf")),
            ("document_file", ("doc1.pdf", b"Test doc 1 content", "application/pdf")),
        ],
    )
